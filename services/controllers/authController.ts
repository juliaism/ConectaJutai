...
export async function login(req: Request, res: Response) {
  ...
  console.log("📱 [LOGIN] Telefone recebido:", phone);
  ...
  try {
    ...
    console.log("✅ [LOGIN] Resultado da busca:", users);
    console.log("⚠️ [LOGIN] Erro da busca:", selectError);

    if (selectError) {
      console.log("❌ [LOGIN] Erro ao buscar usuário:", selectError.message);
      return res.status(500).json({ error: "Erro ao buscar usuário" });
    }

    if (!users || users.length === 0) {
      console.log("❌ [LOGIN] Usuário não encontrado com telefone:", phone);
